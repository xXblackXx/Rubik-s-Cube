...
void Cubies::GenerateNextStates(shared_ptr<Cubies>* NextStates)
{
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][1]))
    {
        _st = Copy(this);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][2]))
    {
        _st = Copy(this);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][3]))
    {
        _st = Copy(this);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][4]))
    {
        _st = Copy(this);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][5]))
    {
        _st = Copy(this);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][0 + 6]))
    {
        _st = Copy(this);
        _st->counterCycle(moves::U);
        NextStates[statesCount++] = _st;
        _st->lastOp = 0 + 6;
    }

    if (lastOp == -1 || (!AvoidTable[lastOp][1 + 6]))
    {
        _st = Copy(this);
        _st->counterCycle(moves::F);
        NextStates[statesCount++] = _st;
        _st->lastOp = 1 + 6;
    }

    if (lastOp == -1 || (!AvoidTable[lastOp][2 + 6]))
    {
        _st = Copy(this);
        _st->counterCycle(moves::D);
        NextStates[statesCount++] = _st;
        _st->lastOp = 2 + 6;
    }

    if (lastOp == -1 || (!AvoidTable[lastOp][3 + 6]))
    {
        _st = Copy(this);
        _st->counterCycle(moves::B);
        NextStates[statesCount++] = _st;
        _st->lastOp = 3 + 6;
    }

    if (lastOp == -1 || (!AvoidTable[lastOp][4 + 6]))
    {
        _st = Copy(this);
        _st->counterCycle(moves::R);
        NextStates[statesCount++] = _st;
        _st->lastOp = 4 + 6;
    }

    if (lastOp == -1 || (!AvoidTable[lastOp][5 + 6]))
    {
        _st = Copy(this);
        _st->counterCycle(moves::L);
        NextStates[statesCount++] = _st;
        _st->lastOp = 5 + 6;
    }
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][0 + 12]))
    {
        _st = Copy(this);
        _st->clockCycle(moves::U);
        _st->clockCycle(moves::U);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][1 + 12]))
    {
        _st = Copy(this);
        _st->clockCycle(moves::F);
        _st->clockCycle(moves::F);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][2 + 12]))
    {
        _st = Copy(this);
        _st->clockCycle(moves::D);
        _st->clockCycle(moves::D);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][3 + 12]))
    {
        _st = Copy(this);
        _st->clockCycle(moves::B);
        _st->clockCycle(moves::B);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][4 + 12]))
    {
        _st = Copy(this);
        _st->clockCycle(moves::R);
        _st->clockCycle(moves::R);
    ...
    if (lastOp == -1 || (!AvoidTable[lastOp][5 + 12]))
    {
        _st = Copy(this);
        _st->clockCycle(moves::L);
        _st->clockCycle(moves::L);
...
Byte Cubies::Heuristic()
{
    ...
    //cout << (int)cornerH << " " << (int)Edge1H << " " << (int)Edge2H << endl;
