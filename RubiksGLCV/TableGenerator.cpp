...
void TableGenerator::CornerTableOrientationGenerator(Byte* CORNER_ORIENT_TABLE)
{
    int f;
    int hashCube;
    int depth = -1;
    ...
    Cube c = Cube(solvedCube);
    Cubies GoalCube = Cubies::Copy(c.toCubiesFromSides());
    Cubies* ResvGoalCube =  new Cubies(GoalCube);
    *ResvGoalCube = Cubies::Copy(GoalCube);

    int countNodes = 0;

    Byte* instack = new Byte[1094];
    memset(CORNER_ORIENT_TABLE, 0, 1094);
    Cubies * currentNode ;
    DataBlock* current ;

    while (countNodes < 2187)
    {
        if (_stack.size() == 0)
        {
            ResvGoalCube = new Cubies(GoalCube); // O(1)
			_stack.push(ResvGoalCube, 0); // O(1)
            depth++;
            memset(instack, 255, 2187);
        }
        ...
        //if ((popcountNodes & 0777777) == 0777777)
        //{
            fprintf(stderr, "\r%d/2187 hashCubeed, on level:%d/11, total traversed:%d", countNodes, depth, popcountNodes);
        //}

        if (current->distance == depth)
        {
            hashCube = currentNode->GetCornerOrientationHash();
            if (hashCube & 1)
            {
                if (!(CORNER_ORIENT_TABLE[(hashCube - 1) / 2] >> 4))
                {
                    CORNER_ORIENT_TABLE[(hashCube - 1) / 2] |= current->distance << 4;
                    countNodes++;
                }
                else
                {
            ...
            {
                if (!(CORNER_ORIENT_TABLE[hashCube / 2] & 15))
                {
                    CORNER_ORIENT_TABLE[hashCube / 2] |= current->distance;
                    countNodes++;
                }
                else
                {
        ...
        else
        {
            ...
            */
            ...
            currentNode->TableGenerateNextStates(NextStates);


            for (int i = 0; i < currentNode->statesCount; i++)
            {

                hashCube = NextStates[i]->GetCornerOrientationHash();
